...
data_list_id<-"ff7308f6-1aed-46d5-988d-354fd0cdc4f6"
...
features_list <- list("office" = "diplomatic")
...
extra_columns <- c("consulate", "country", "target", "diplomatic", "embassy","liaison", "office")
...
cleaned as (SELECT
            ...
            jsonb_strip_nulls(jsonb_build_object(
              'und', CASE WHEN name IS NULL THEN 'embassy' ELSE name END,
              'fre', name_fr,
              'ger', name_de::text,
              'dut', name_nl)) as name,
            jsonb_build_object(
...
### Create fdw views ----
fdw_views_sql <- c("
DROP VIEW IF EXISTS fdw.fdw_embassy CASCADE;
","
CREATE OR REPLACE VIEW fdw.fdw_embassy
AS
SELECT id,
original_id,
name,
...
FROM transformation.embassy;
","
ALTER TABLE fdw.fdw_embassy
OWNER TO paragon;
","
GRANT SELECT ON TABLE fdw.fdw_embassy TO fdw4paragon;
","
GRANT ALL ON TABLE fdw.fdw_embassy TO paragon;
")
...
run_smart_update = function() {
  smart_update_process("embassy", 50, 100, 50, format(Sys.Date(), "%Y-%m-%d"), update_even_if_checks_fail)
}
